<?php
// Делаем класс абстрактным, чтобы нельзя было создавать объекты.
abstract class Model{
	const TABLE = ''; //имя таблицы модели

	protected $id; // Поле id есть у каждой модели.
	...
	public function __construct($id = null) {
		...
		if ($id) {
            ...
            if ($row = Db::getRow('SELECT `' . implode('`, `', array_keys(static::$fields_description)) . '` FROM `' . static::TABLE . '` WHERE `id` = ' . $id)) {
                $this->id = $id;
